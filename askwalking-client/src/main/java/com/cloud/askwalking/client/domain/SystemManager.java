...
public class SystemManager {
    ...
    public static int getSystemCode() {
        if (environment == null) {
            log.warn("[警告]请先调用初始化：SystemManager.initEnvironment()");
            return 0;
        }
        String code = environment.getProperty(GlobalConstant.SYSTEM_CODE);
        if (code == null) {
            log.warn("[警告]没有设置系统码 application.properties ->'system.code'");
            return 0;
        }
    ...
    public static String getEnv() {
        ...
        if (StringUtils.isBlank(env)) {
            log.error("没有获取到有效的环境->spring.profiles.active=[local,dev,test,prepare,product]");
        }
        return env;
    }
    ...
    public static String getOpenSecret() {
        ...
        if (open_secret == null) {
            log.info("没有获取到配置`{}`", GlobalConstant.OPEN_SECRET_CONFIG_KEY);
        }
        return open_secret;
    }
